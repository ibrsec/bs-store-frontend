...
const AdminProductCard = ({ product,setOpen,setEditItem }) => {
  ...
  return (
    <Grid item xs={12} sm={6} md={4} lg={3} >
      <Card>
        <Box onClick={()=>navigate("/detail/"+product?._id)} sx={{cursor:'pointer'}}>
        <CardMedia
          component="img"
          alt={product?.title}
          height="140"
        ...
        <CardContent>
          ...
          <Typography variant="body2" lineclamp={2} color="textSecondary" component="p">
            {product?.description}
          </Typography>
        ...
        <Box display={"flex"} gap={1} justifyContent={"center"} mb={2}>
            ...
            <Button variant="contained" color='warning' onClick={()=>deleteProduct(product?._id)}>Delete</Button>
        </Box>
      </Card>
    </Grid>
  );
